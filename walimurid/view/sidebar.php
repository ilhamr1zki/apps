  ...
  <div class="box-footer no-padding">
    <ul class="nav nav-stacked">
      <?php $lihtip=mysqli_query($con,"SELECT * FROM tipenilai order by p asc ");while($hlihtip=mysqli_fetch_array($lihtip)){ 
        $nil=mysqli_fetch_array(mysqli_query($con,"SELECT sum(nilai) as nilai FROM nilai where c_ta='$c_ta' and c_tipenilai='$hlihtip[c_tipenilai]' and c_siswa='$siswa[c_siswa]' and c_kelas='$siswa[c_kelas]' "));
        if($nil['nilai']<1){echo '<li><a><b>Nilai '.$hlihtip['tipe'].'('.$hlihtip['ket'].')</b></a></li>';}else{
          echo '<li><a><b>Nilai '.$hlihtip['tipe'].'('.$hlihtip['ket'].')<span style="float:right;">: '.$nil['nilai'].'</span></b></a></li>';
        }                      
      } ?>
    </ul>
  </div>
