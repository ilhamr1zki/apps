...
$pdf->SetX(4);
$pdf->Cell(10,0.5,'  Total Nilai Siswa Kelas '.$kel['kelas'],0,'L');
...
$lsis=mysqli_query($con,"SELECT * FROM siswa where c_kelas='$kelas' order by nama asc "); $no=1; 
  ...
  $pdf->Cell(6.5, 1, ' '.$hlsis['nama'], 1, 0, 'L');
    ...
    while($hlihtip=mysqli_fetch_array($lihtip)){
      $nil=mysqli_fetch_array(mysqli_query($con,"SELECT sum(nilai) as nilai FROM nilai where c_ta='$c_ta' and c_tipenilai='$hlihtip[c_tipenilai]' and c_siswa='$hlsis[c_siswa]' and c_kelas='$kelas' "));
      $pdf->Cell($l, 1, $nil['nilai'], 1, 0, 'C');
      
    }
  $nilsemua=mysqli_fetch_array(mysqli_query($con,"SELECT sum(nilai) as nilai FROM nilai where c_ta='$c_ta' and c_siswa='$hlsis[c_siswa]' and c_kelas='$kelas' "));
