...
<?php } ?>
        <div class="box-body">
          ...
          <div class="row">
            <?php $ex=mysqli_query($con,"SELECT * FROM extra order by extra asc ");while($hex=mysqli_fetch_array($ex)){ $ckex=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM extrasiswa where c_ta='$rs[c_ta]' and c_siswa='$ss[c_siswa]' and c_extra='$hex[c_extra]' ")); if($ckex==NULL){$ced='';}else{$ced='checked';} ?>
            <div class="col-md-3"><label><input type="checkbox" <?php echo $ced; ?> name="extra[]" value="<?php echo $hex['c_extra']; ?>"> <?php echo $hex['extra']; ?></label></div>
            <?php } ?>
          </div>
          <br>
          <p>Ketidakhadiran</p>
