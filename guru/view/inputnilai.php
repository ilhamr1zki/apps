              ...
              <table id="example1" class="table table-bordered table-hover">
                ...
                <?php $lsis=mysqli_query($con,"SELECT * FROM siswa where c_kelas='$_GET[q]' order by nama asc "); $no=1; while($hlsis=mysqli_fetch_array($lsis)){
                  ...
                  <tr>
                    ...
                    <td><?php echo $hlsis['nama']; ?></td>
                    <td>
                      <input style="width:80px;" type="" onkeyup="numberbox()" name="nilsiswa[]" class="form-control" onkeypress="return hanyaangka(event)" value="<?php if($amnil['nilai'] ?? 0 <1){}else{ echo $amnil['nilai'] ?? 0; } ?>">
                    </td>
                  </tr>
                <?php $no++; } ?>
                </tbody>
